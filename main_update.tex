...
% Versão adaptada ao modelo da Sociedade Brasileira de Computação (SBC)
% =============================================================================
...
\title{Sistema para Controle de Eventos Acadêmicos:\\
Modelagem, Implantação e Avaliação de um Banco\\
de Dados Relacional em PostgreSQL}

\author{Deivid Costa dos Santos\inst{1},
        Douglas Aleixo de Santana\inst{1},
        Emanuel Patrick Alves Bastos\inst{1},\\
        Igor Spinola de Albuquerque e Almeida Teles\inst{1},
        Pedro Vitório Larchert de Oliveira\inst{1}}

\address{Instituto de Computação --
  Universidade Federal da Bahia (UFBA)\\
  Avenida Milton Santos, s/n -- Ondina -- 40.170-110 -- Salvador -- BA -- Brasil
  \email{\{deivid.santos,douglas.santana,emanuel.bastos,igor.teles,
  pedro.larchert\}@ufba.br}
}
...
relational database for the management of academic events, developed
in the context of the MATA60 -- Database course at the Federal
University of Bahia. The system covers the full lifecycle of a
scientific event, from registration and payment control to paper
submission, peer review and proceedings publication. The conceptual
model was elaborated using Peter Chen's notation in BrModelo and then
translated into a relational schema deployed on PostgreSQL~16 within a
Docker environment. Ten intermediate queries and twenty advanced
queries were developed, together with an indexing plan whose
performance is empirically compared.
\end{abstract}

\begin{resumo}
Este trabalho descreve o desenvolvimento de um banco de dados
relacional voltado ao gerenciamento de eventos acadêmicos, elaborado
no âmbito da disciplina MATA60 --- Banco de Dados, da Universidade
Federal da Bahia. O sistema contempla o ciclo completo de um evento
científico, desde o controle de inscrições e pagamentos até a
submissão, revisão por pares e publicação nos anais. O modelo
conceitual foi elaborado com a notação de Peter Chen na ferramenta
BrModelo e posteriormente traduzido para o modelo relacional, com
implantação no PostgreSQL~16 em ambiente Docker. Foram desenvolvidas
dez consultas intermediárias e vinte consultas avançadas, além de um
plano de indexação cuja eficiência é avaliada empiricamente.
\end{resumo}
...
\section{Anexo D --- Tutorial de Reprodução}
\label{anx:tutorial}
